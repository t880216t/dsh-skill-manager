...
- **套件更新**：每次启动用 `git ls-remote` 取套件仓库分支 head，与上次同步记录一致就整个跳过（零下载）；有新提交则覆盖同步——远端变化的条目替换本地副本，替换前若该条目与上次同步的指纹不符（用户改过，或是本功能之前装的没有账），先整目录备份到 `~/.dsh/skill-backups/<时间戳>/<条目>`；远端删除且本地未改的条目移除，改过的保留原地不动。面板里套件仓库那一行有「更新套件」按钮可手动触发（跳过 revision 短路）。用户自己添加的技能不在同步账本里，因此永不被触碰；行内配置 `autoUpdate: false` 可关闭启动检查。
- **预置仓库**改为内部 Supertester 套件仓库，跟踪 `main` 分支；状态文件里记着已被取代的分支（`master`、`feat/skill-only-architecture-2.0`）的装机在下次启动迁移到 `main`，用户自己添加的仓库不动。
