...
import { updateSuiteRepos } from "./suite-sync.js";
...
/**
 * 曾经配置过的套件分支。套件是按状态文件里记的分支克隆的（不是按
 * DEFAULT_REPOS），所以每个被取代的旧值都必须列在这里，否则装过旧版的机器
 * 会一直拉旧分支。
 */
const OBSOLETE_SUITE_BRANCHES = ["master", "feat/skill-only-architecture-2.0"];

/** 把预置套件条目上已被取代的分支迁到当前分支，其他已保存仓库不动。 */
function migrateDefaultRepos(repos) {
  let changed = false;
  const defaults = DEFAULT_REPOS[0];
  const migrated = repos.map((repo) => {
    if (defaults !== undefined
      && repo.host === defaults.host
      && repo.owner === defaults.owner
      && repo.name === defaults.name
      && repo.suite === true
      && typeof repo.branch === "string"
      && OBSOLETE_SUITE_BRANCHES.includes(repo.branch)) {
      changed = true;
      return { ...repo, branch: defaults.branch };
    }
    return repo;
  });
  ...
  try {
    ...
    if (parsed !== null && typeof parsed === "object") {
      ...
      if (repos === undefined) {
        ...
        const migrated = migrateDefaultRepos(repos);
...
const suiteUpdateResultSchema = z.object({
  repos: z.array(
    z.object({
      owner: z.string(),
      name: z.string(),
      ...
      backedUp: z.array(z.object({ name: z.string(), path: z.string() }))
    })
  ),
  failures: z.array(z.object({ repo: z.string(), error: z.string() }))
});
...
const MANIFEST = {
  ...
  invocations: [
    ...
    },
    {
      ...
      invocation: { kind: "direct" },
      parameters: [
        { name: "owner", wire: "owner", source: "json", acceptsUndefined: true, codec: { mode: "strict", typeSymbol: "dsh-skill-manager#RepoOwner", schema: z.string().optional() } },
        { name: "name", wire: "name", source: "json", acceptsUndefined: true, codec: { mode: "strict", typeSymbol: "dsh-skill-manager#RepoName", schema: z.string().optional() } }
      ],
      result: { mode: "strict", typeSymbol: "dsh-skill-manager#SuiteUpdateResult", schema: suiteUpdateResultSchema }
    }
  ],
...
class SkillManagerGateway extends TypertRemoteService {
  ...
  /**
   * 覆盖式更新套件仓库：远端变化的条目替换本地副本，替换前把用户改过的
   * 条目整目录备份；远端删掉且本地未改的条目移除。用户自己添加的技能从不
   * 在同步账本里，因此永不被触碰。
   * @param {string} [owner] 只更新这一个仓库（省略则全部套件仓库）
   * @param {string} [name] 与 owner 成对给出
   */
  async updateSuite(owner, name) {
    const state = await loadState();
    const repos = state.repos.filter((repo) => repo.suite === true
      && (owner === undefined || (repo.owner === owner && repo.name === name)));
    return await updateSuiteRepos({
      dshHome: resolveDshHome(),
      destRoot: this.userSkillsRoot(),
      repos,
      // 手动更新必须真的去看远端，不能被 revision 短路
      force: true,
      resolveSource: (repo) => this.repoSource(repo.owner, repo.name, repo.branch),
      log: (message) => this.ctx?.logger?.info?.(message) ?? console.log(message)
    });
  }
}
...
export function apply(ctx, config = {}) {
  ...
  // 启动流程，两步串行（它们写同一个技能根，并发会互相看见半装状态）：
  //   1. 首次启动的一次性整树安装（config.autoInstall === false 关闭）
  //   2. 每次启动的套件更新（config.autoUpdate === false 关闭）——远端
  //      revision 与上次同步一致时整个跳过，不产生下载
  // 任一步失败只记录，绝不阻断应用启动。首次启动两步都会拉一次树：更新
  // 那次全部命中"内容一致"，只为把条目指纹建账，之后的启动才有据可依。
  void (async () => {
    const dshHome = resolveDshHome();
    if (config.autoInstall !== false) {
      const state = await loadState();
      await runAutoInstallOnce({
        dshHome,
    ...
    const state = await loadState();
    await updateSuiteRepos({
      dshHome,
      destRoot: gateway.userSkillsRoot(),
      repos: state.repos,
      resolveSource: (repo) => gateway.repoSource(repo.owner, repo.name, repo.branch),
      log: (message) => ctx.logger?.info?.(message) ?? console.log(message)
    });
  })().catch((err) => {
    console.warn(`skill-manager: 启动自动安装/更新意外失败：${err?.message ?? err}`);
  });
}
